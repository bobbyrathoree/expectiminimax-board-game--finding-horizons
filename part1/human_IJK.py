...
def next_move(game: Game_IJK) -> None:
    ...
    print(
        "Please enter move for player %s (U, D, L, R, or S): " % game.getCurrentPlayer()
    )
    move = input().strip().upper()
    while move not in "UDLRS":
        print(
            "Bad move! Please try again and enter move for player %s (U, D, L, R, or S): "
            % turn
        )
        move = input().strip()
